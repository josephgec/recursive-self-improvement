"""Hindsight target experiment: what should hindsight rationalization target?"""
...
class HindsightTargetExperiment(Experiment):
    """Experiment testing different hindsight rationalization targets."""

    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__(config)
        self.name = "hindsight_target"
        self.iterations = (config or {}).get("iterations_per_condition", 20)

    def get_conditions(self) -> List[HindsightCondition]:
        return build_hindsight_conditions(total_iterations=self.iterations)

    def configure_pipeline(self, condition: HindsightCondition, pipeline: Any) -> Any:
        """Configure the pipeline with this condition's hindsight target policy."""
        pipeline.set_hindsight_policy(condition.policy)
        pipeline.set_condition_name(condition.name)
        return pipeline

    def measure(self, pipeline: Any, condition: HindsightCondition) -> ConditionResult:
        """Run the pipeline and measure performance under this hindsight target."""
