"""RLM recursion depth experiment: how deep should recursive self-improvement go?"""
...
class RLMDepthExperiment(Experiment):
    """Experiment testing different recursion depths for the RLM."""
    ...
    def get_conditions(self) -> List[DepthCondition]:
        return build_depth_conditions()

    def configure_pipeline(self, condition: DepthCondition, pipeline: Any) -> Any:
        """Configure the pipeline with this condition's recursion depth."""
        pipeline.set_depth(condition.depth)
        pipeline.set_condition_name(condition.name)
        return pipeline

    def measure(self, pipeline: Any, condition: DepthCondition) -> ConditionResult:
        """Run the pipeline and measure performance at this recursion depth."""
        ...
        return ConditionResult(
            ...
            metadata={"depth": condition.depth},
        )

    @staticmethod
    def theoretical_accuracy(depth: int) -> float:
        """Theoretical accuracy as a function of depth: 0.55 + 0.17*(1-exp(-depth/1.5))."""
        return 0.55 + 0.17 * (1.0 - math.exp(-depth / 1.5))

    @staticmethod
    def theoretical_cost(depth: int) -> float:
        """Theoretical cost as a function of depth: 0.005 * 2^depth."""
        return 0.005 * (2 ** depth)
